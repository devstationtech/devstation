...
describe("cluster.proxmox.images.unassign endpoint — integration", () => {
  let rpc: Client;
  ...
  it("removes the image association from the node", async () => {
    /* @When the image is unassigned from the node */
    await rpc.invoke<ClusterProxmoxImagesUnassignResponse>(
      "cluster.proxmox.images.unassign",
      { sessionId: STUB_SESSION_ID, clusterId, nodeId, imageId: IMAGE_ID },
    );

    /* @Then the node has no remaining image assignments */
    const records = await persistence.readClusters();
    const target = records.find((c) => c.id === clusterId)!;
    assertEquals(target.nodes[0].images ?? [], []);
  });
});
