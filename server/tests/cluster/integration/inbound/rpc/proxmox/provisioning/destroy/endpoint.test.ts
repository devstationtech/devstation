...
import { DestroyEndpoint } from "@server/cluster/inbound/rpc/proxmox/provisioning/destroy/endpoint.ts";
...
class StubProvisioning implements Provisioning {
  ...
  destroy(_cluster: ProxmoxCluster, _nodeIds: NodeId[]): Task {
    return {
      run: (_op, emitter) =>
        emitFrom(
          (async function* (): AsyncIterable<DomainExecutionEvent> {
            yield new Step("destroy", "web/prod");
            yield new Log("Destroying...");
            yield new Log("✓ web/prod: -2");
          })(),
          emitter,
        ),
    };
...
describe("cluster.proxmox.provisioning.destroy endpoint — integration", () => {
  let rpc: Client;

  beforeAll(() => {
    const operations = new InMemoryExecutions();
    const handler = new DestroyNodesHandler(
      new StubClusters(),
      operations,
      new StubProvisioning(),
      new StubDispatcher(),
    );
    const endpoint = new DestroyEndpoint(handler);
  ...
  it("acknowledges the request with an executionId so the UI can attach via operation.watch", async () => {
    /* @When the client invokes cluster.proxmox.provisioning.destroy */
    const response = await rpc.invoke<ClusterProxmoxProvisioningDestroyResponse>(
