...
import type { ClusterProxmoxProvisioningApplyResponse } from "@jsonrpc-contracts-ts/cluster.gen.ts";
import { InMemoryExecutions } from "@server/shared/executions/outbound/in-memory-executions.ts";
import { ApplyEndpoint } from "@server/cluster/inbound/rpc/proxmox/provisioning/apply/endpoint.ts";
...
class StubProvisioning implements Provisioning {
  ...
  apply(_cluster: ProxmoxCluster, _nodeIds: NodeId[]): Task {
    return {
      run: (_op, emitter) =>
        emitFrom(
          (async function* (): AsyncIterable<DomainExecutionEvent> {
            yield new Step("apply", "web/prod");
            yield new Log("Applying changes...");
...
describe("cluster.proxmox.provisioning.apply endpoint — integration", () => {
  let rpc: Client;

  beforeAll(() => {
    const operations = new InMemoryExecutions();
    const handler = new ApplyNodesHandler(
      new StubClusters(),
      operations,
      new StubProvisioning(),
      new StubDispatcher(),
    );
    const endpoint = new ApplyEndpoint(handler);
  ...
  it("acknowledges the request with an executionId so the UI can attach via operation.watch", async () => {
    /* @When the client invokes cluster.proxmox.provisioning.apply */
    const response = await rpc.invoke<ClusterProxmoxProvisioningApplyResponse>(
      "cluster.proxmox.provisioning.apply",
      { sessionId: SESSION_ID, clusterId: "homelab", nodeIds: ["web"] },
    );
