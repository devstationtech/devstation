...
describe("cluster.proxmox.virtualMachine.update endpoint — integration", () => {
  let rpc: Client;
  ...
  it("replaces the VM size mutable fields", async () => {
    /* @When the VM is updated with a new name, address, resources and tags */
    await rpc.invoke<ClusterProxmoxVirtualMachineUpdateResponse>(
    ...
    /* @Then the stored VM reflects the updated mutable fields */
    ...
    assertEquals(virtualMachines[0].name, "test-vm-renamed");
    assertEquals(virtualMachines[0].address, "10.0.0.101");
    assertEquals(virtualMachines[0].resources.cpu, 4);
    assertEquals(virtualMachines[0].resources.ram, 2048);
    assertEquals(virtualMachines[0].tags, ["k3s", "renamed"]);
  });
});
