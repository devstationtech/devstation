...
/**
 * Renames a vault in place — the id (and every reference to it) is preserved,
 * unlike delete + recreate. Mirrors `CreateVaultForm`; the field is pre-filled
 * with the current name.
 */
export function RenameVaultForm({ vaultId, currentName, onBack, onRenamed }: Props) {
  ...
  const handleSubmit = (values: Record<string, string>) => {
    setName(values.name);
    setSubmitting(true);
    vault.renameVault({ sessionId, vaultId, name: values.name })
      .then(() => {
        setSubmitting(false);
        setResult({ type: "success", summary: `Vault renamed to '${values.name}'` });
      })
      .catch((err: Error) => {
        setSubmitting(false);
  ...
  return (
    <ScreenFrame
      breadcrumb={["topologies", "vaults", currentName, "rename"]}
      intent={intent}
      topRight={topRight}
      secondary={secondary}
      footer={footer}
