...
export function CreateVaultForm({ onBack, onCreated }: Props) {
  ...
  const handleSubmit = (values: Record<string, string>) => {
    ...
    vault.createVault({
      ...
      .then(() => {
        setSubmitting(false);
        setResult({ type: "success", summary: `Vault '${values.name}' created` });
      })
      .catch((err: Error) => {
        setSubmitting(false);
  ...
  return (
    <ScreenFrame
      breadcrumb={["topologies", "vaults", "create"]}
      intent={intent}
      topRight={topRight}
      secondary={secondary}
      footer={footer}
    >
      {submitting ? <Spinner label={`creating vault '${name}'...`} /> : (
        <Form
          fields={[
            {
              type: "string",
